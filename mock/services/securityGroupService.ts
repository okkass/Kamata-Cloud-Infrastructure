...
export const getSecurityGroups = (): Array<SecurityGroupResponse> => {
...
export const updateSecurityGroup = (
  ...
  let target = getSecurityGroupById(id);
  if (target === undefined) {
    return undefined;
  }
...
export const deleteSecurityGroup = (id: string): boolean => {
...
export const getSecurityRules = (
  ...
  const securityGroup = getSecurityGroupById(id);
  return securityGroup?.rules;
};

export const getSecurityRuleById = (
  sgId: string,
  ruleId: string
): SecurityRuleResponse | undefined => {
  const securityGroup = getSecurityGroupById(sgId);
  return securityGroup?.rules.find((rule) => rule.id === ruleId);
};

export const addSecurityRule = (
  sgId: string,
...
export const deleteSecurityRule = (sgId: string, ruleId: string): boolean => {
  const securityGroup = getSecurityGroupById(sgId);
  if (!securityGroup) {
    return false;
  }
