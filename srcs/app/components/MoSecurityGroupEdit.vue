<template>
  <BaseModal :show="show" title="セキュリティグループの編集" @close="$emit('close')">
    <div class="space-y-4">
      <form @submit.prevent="submit" class="space-y-4">
        <div>
          <label for="sg-name" class="block text-sm font-medium text-gray-700">グループ名</label>
      ...
      <div class="pt-4 border-t">
        <h3 class="text-lg font-semibold text-gray-800">渡された全グループ一覧（確認用）</h3>
        <!-- preタグを使うと、オブジェクトを整形して表示できるのでデバッグに便利 -->
        <pre class="mt-2 p-2 bg-gray-100 rounded-md text-xs overflow-x-auto"><code>{{ allSecurityGroups }}</code></pre>
      </div>
...
defineProps({
  ...
  allSecurityGroups: {
    type: Array as PropType<SecurityGroupDTO[]>,
    default: () => [],
  },
});
